...
飞行雪绒是面向 Windows 10/11 的桌面宠物项目。当前主线以 `LTS1.0.7pre4V2` 为版本基线，核心能力包括桌宠展示、事件驱动对象系统、AI 伴聊、语音播报、本地语音识别、多源音乐播放、粒子与小游戏扩展。
...
- 更新器只探测能放下离线安装器 ZIP 的 Hugging Face 与 ModelScope 两个更新源（与原生在线
  安装器一致），共享 16 秒总上限，先到者优先；随后下载并校验外层 ZIP 中的离线安装器 EXE。
  用户确认后退出桌宠，由原生安装器完成目录切换，不执行本机 Python 或外部 Node。
...
- CUDA、Torch、NVIDIA、TensorRT 不进入基础发行版；DirectML 以独立 overlay 随包提供，
  由设置面板按需启用。
- 安装器使用原生 ZIP 解压和尾部 SHA-256 校验：包内条目承载路径与真实大小，字节则存在
  4 路固态 LZMA2 分片里，解压按实测负载自适应并行、不吃满单个核心。安装过程显示当前
  文件、百分比、文件/字节进度和预计剩余时间；安装结束后由用户点击“退出安装并启动飞行雪绒”。
