...
`cmake/embed_ptx.cmake` 曾经是构建里最慢的一步：它按两个十六进制字符循环一次（半兆的
内核对应上百万次），每轮都把整份十六进制串展开进一次 `string(SUBSTRING ...)`，再把
8 KB 的缓冲逐字符 `string(APPEND)`，两次拷贝都是平方级，实测二十分钟以上还没跑完。
现在用一次正则把每个字节转义、再按 8192 个字符切块（块长是 4 的倍数，不会切断一个
`\xNN` 转义），同一份 542 KB 的 PTX 由 2.4 秒生成完，解码回来与原文件逐字节一致。
...
- 跨图的那一段也不落主机：前一半解码器把缓存声明成「只留在卡上、不与本图输入配对」的
  输出（`fsv_graph_set_retained`），后一半借走这块显存并绑到自己 `past_*` 的输入名上
  （`fsv_graph_borrow_device`/`fsv_graph_import_device`），跑完交还；解码器每步回灌的
  `y`/`y_emb` 也按同一张配对表留在卡上。见“跨图设备缓冲直通”。
...
- 池的键是 size class，不是 1 KiB 粒度向上取整后的字节数：256 KiB 及以下仍按 1 KiB
  对齐，之上按 1/8 递增（`pool_class()`），256 KiB–64 MiB 之间只有约 47 档、最多浪费
  12.5%。解码每步的序列都比上一步长，同一族中间张量的尺寸几乎不重样，1 KiB 粒度的桶
  等于每次都落空；按 1/8 递增后相邻几步能共用一个缓冲。31 字基准里含载入的一次进程
  真实分配由 10326 次降到 7020 次（池命中 42227 → 45533），长文本收益见
  “长文本与显存池的 size class”。
...
### broadcast 逐元素内核的行内快路径（本轮）

`fsv_binary_bcast_f32` 原来一个线程只算一个输出，每算一个都要走一次坐标回代，而回代是
一串软件除法。语音包里这个内核的绝大多数调用是「逐通道偏置」——`[1,C,1]` 对 `[1,C,T]`，
内层维度上一侧是广播（步长 0）、一侧是连续（步长 1）——所以一行之内两个操作数的偏移都是
同一个常量的倍数。

现在一个线程算 4 个相邻输出：会跨行的那一组退回逐元素回代，行内的一组只回代一次，
两侧各自按步长 0/1 取值——步长为 1 且偏移 16 字节对齐的一侧读 float4，广播侧只读一次，
结果一次 float4 写出。

`FSV_NV_GPU_TIMER`（同一份 int8 包、653 次启动、形状完全不变）量到该内核由 **0.312 s
降到 0.097 s**（两次合计算，3.2×）。同一轮里 `fsv_copy_contiguous_f32`、
`fsv_unary_ops_f32` 等其它内核反而慢了 10%–30%（长时间基准之后机器降频），所以这是一次
可归因的内核改善，不是机器抖动；`fsv_engine_smoke` 改动后全部 PASS。

`fsv_kernel_bench` 新增用例 `binary_bcast batch channels spatial`。**这个用例的绝对值不能
用来判断收益**：在 `1×512×18508` 这种九百万元素的合成形状上该内核本来就是访存受限，
改动前后都是约 22 GB/s、没有差别，真正受益的是语音包里那种三十几万元素、每行都很短的
调用。判断收益用真实合成里的逐内核计时。
...
31 字的基准任务是约 90000 个节点、约 54500 次内核启动、约 3800 次真实显存分配；
256 字（46.46 s 音频）一次合成是 1053882 次内核启动、235719 次真实分配、51618 次上传
1.90 GB、6482 次回读 1.25 GB。内核启动只多一个数量级，真实分配多了两个数量级——
多出来的那一份全落在显存池上。原因是池原来按 1 KiB 粒度向上取整当键，而解码每步的
序列都比上一步长，同一族中间张量的字节数几乎不重样，「最小的够用缓冲」几乎每次都
落空，每个张量各来一次 `cuMemAlloc`/`cuMemFree`（各自掐断一次上下文）。

`pool_class()` 把键改成 size class：256 KiB 及以下仍按 1 KiB 对齐，之上按 1/8 递增，
256 KiB–64 MiB 之间约 47 档、最多浪费 12.5%。两版 DLL 在同一台机器上交替测量
（同一份 256 字文本、同一 seed）：

| | 真实分配 | 分配段 | launch 段 | 端到端 |
| --- | --- | --- | --- | --- |
| 基线（1 KiB 桶） | 235719 | 18.78 s | 18.41 s | 110.11 s |
| size class | 115512 | 7.62 s | 11.58 s | 67.27 s |

端到端不能直接相减：这台笔记本的后台进程能把同一份 256 字任务的墙钟在 67 s–150 s
之间拉。`launch` 段和分配段面对的是同一份工作量，可以当机器速度的探针：五次测量里
`alloc/launch` 基线是 1.02–1.21、size class 是 0.658–0.682，两组的取值范围不重叠，
即同等工作量下分配段稳定省掉约 0.5 个 launch 段——按这台机器的状态折合一次 256 字
合成约 7 s。同一轮里 `launch` 次数（1053882）、上传与回读的次数和字节数两版完全
相同，说明改动只动了分配路径。31 字基准同样下降：含载入的一次进程真实分配
10326 → 7020 次（池命中 42227 → 45533），端到端 8.54 s → 8.12 s（−4.9%，
同机连续测量，在抖动范围内）。

长文本的下一块收益不在池上，而在形状链：一次合成里约 13000 个 int64 形状张量留在
主机（见“开销分解”），它们同时撑高了分配次数与主机分派，把它们在准备阶段折掉是
现在最大的一块。

### 值表与执行计划

节点循环原来每个操作数都要在 `unordered_map<std::string, Value>` 里按名字查一次、每个结果
按名字写一次，而每一次图执行（解码的每一步就是一次）都要重建整张值表、再全图统计一遍
引用次数。一次合成的节点数是几万到几十万，这些账是按节点计的。

现在 `prepare()` 期做一次解析（`build_plan()`）：

- 把模型能提到的每个名字定到一个值槽——包括 If 分支与 Loop 体会写回外层作用域的输出名；
- 记下每个节点的操作数槽位与结果槽位，空名字记成“没有这个操作数”；
- 记下每个槽位被读的次数（子图的读也计进外层，与原来的 `count_value_uses()` 同一口径），
  图输出记为永不释放，因为调用方在图结束后还要读它。

运行期只剩三件事是按节点计的：按槽位取操作数、按槽位写结果、把读计数减到零时归还该值的
显存缓冲。If 分支和 Loop 体仍然走原来的名字表——它们每次合成都只有个位数次，不值得为它们
引入第二套寻址；子图作用域因此保持原语义不变。

`FSV_NATIVE_PROBE=1` 把节点循环的墙钟拆成“取输入 / 执行 / 存结果”三段，用来判断下一次
优化该往哪里放。31 字基准（两版 DLL 交替各三次，同一份工作量的节点数都是 217485）：
同进程 `cpu/cuda` 比值 0.791 → 0.739、cuda 侧 1.66 s → 1.56 s，其中取输入 0.173 → 0.081 s、
存结果 0.243 → 0.043 s；执行段基本不动（2.37 → 2.46 s，在抖动内）。
**读这三个数的时候注意：`exec` 里包含每次核函数启动的入队时间，它才是剩下的大头。**

### 算子派发的整数比较

`build_plan()` 顺带把每个节点的算子名折成 64 位 FNV-1a 哈希（`hash_op_name()`，结果存进
`Plan::op_hashes`），节点循环里的分派因此从字符串比较变成整数比较，而字面量在编译期就折好
了。If/Loop 体不进计划，仍按名字现场哈希一次，语义与那一路的开销都保持原样。74 个算子名的
哈希实测 74/74 不重复，`fsv_engine_smoke` 与全部单元测试通过。

### 跨图设备缓冲直通

解码器分两半：前一半（`t2s_first_stage`）产出 KV 缓存，后一半（`t2s_stage`）把它当作自己的
`past_*` 吃进去。这份缓存本来就在显存里，但两半是两个图、两个句柄，原来的走法是「回读主机
→ 再上传」，每步白跑一趟。三个入口把这段路接上：

- `fsv_graph_set_retained()` 声明「只留在卡上、不与本图的输入配对」的输出。运行期照常回
  名字、形状和类型，载荷为空；真要看字节时走 `fsv_graph_read_resident()`；
- `fsv_graph_borrow_device()` 把这样一个输出的显存缓冲借出去，`fsv_graph_import_device()`
  把它绑到消费者自己的输入名上（借来的缓冲没有主机载荷，形状和类型随借据一起走），
  `fsv_graph_release_device()` 交还。`GraphRuntime::run()` 消费一次就丢掉，不留给下一轮；
- 借不到——那个值没有显存副本，或者运行时不认识这几个入口（`native_graph.py` 按可选入口
  绑定，老 DLL 不会因此加载失败）——就退回按字节走，语义不变。

配对表在 `native_graph.py`，除了 `present_*`↔`past_*` 这条缓存规则还有第二条：解码器每步把
自己的 `y`/`y_emb` 回灌成下一步的 `iy`/`iy_emb`。它们随步数变长，256 字一次合成里是 GB 级的
来回，比缓存那一段更值钱。这条规则只在图同时有 `past_*`/`present_*` 时才生效：有缓存的才是
这种自回归解码器，别的图不受影响。

同一台机器、同一个 DLL、同一份 256 字文本（46.46 s 音频、seed 20260910）交替各一次：

| | 上传 | 回读 | 真实分配 | launch | 驱动侧合计 | 墙钟 |
| --- | --- | --- | --- | --- | --- | --- |
| 只驻留缓存 | 51522 次 1.82 GB | 6386 次 1.17 GB | 139598 次 8.30 s | 1053882 | 25.30 s | 67.21 s |
| 加上设备直通 | 50255 次 0.68 GB | 5117 次 0.03 GB | 151651 次 8.47 s | 1053882 | 23.96 s | 67.55 s |

`launch` 次数一字不差，说明改动只动了搬运路径；一次合成少运 2.28 GB，回读字节从 1.17 GB
掉到 0.03 GB。真实分配涨了 8.6%（池命中 900797 → 887477）：回灌的 `y`/`y_emb` 每步都比
上一步长，它们的缓冲和别的中间张量一样进不了同一个 size class，分配段 8.30 → 8.47 s、
峰值显存 2798.1 → 2796.6 MB，按抖动看这一项没有变。墙钟 67.21 s / 67.55 s 也在这台机器
63–150 s 的抖动里，短句那一段才有同进程比值可用：31 字基准回读 839 → 683 次、上传
12637 → 12484 次，同进程 cpu/cuda 比值 0.709 → 0.72（两版都在抖动带内）。

两次运行的 wav SHA256 都是 `84129F7C…C536E833`，与 256 字任务的既有基准逐位一致；31 字
基准里 cpu 与 cuda 的采样点 60800/60800 一致、最大绝对误差 6.1e-05。
...
5. 1×1 卷积与 batched MatMul。`m == 1` 的 batched MatMul 本轮换成了 GEMV 快路径：
   ...
   1.4×–2.6×。约 3.6 万次逐元素/拷贝/转置启动里，broadcast 逐元素内核本轮也改成每线程
   4 个输出（见下文“broadcast 逐元素内核的行内快路径”），真实合成里该内核 0.312 s →
   0.097 s；剩下的逐元素与拷贝启动仍按这一条往下收。
6. 再决定前端图与默认后端的切换。
