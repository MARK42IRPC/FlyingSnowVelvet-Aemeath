...
合成的驱动侧开销（`FSV_CUDA_STATS`）是 launch 60931 次 0.56 s、真实显存分配 19947 次
0.53 s、上传 9356 次 1.46 GB 0.63 s、回读 672 次 0.10 GB 0.72 s，合计 2.45 s；内核侧
按 `fsv_kernel_bench` 的单价折算已只剩 1 s 以内。这两个包的 GPU 大部分时间在等主机，
继续压内核收益有限，下一步的收益要从主机侧拿。
...
1. 主机侧：int8 包每次合成有 54500 次启动、4900 次上传、90000 个节点的解释器分派，
   fp32/fp16 包还要多背 1.46 GB 上传；把只在形状上做算术的 int64 子图在准备阶段
   折掉能同时打这几项，这是现在最大的一块。
