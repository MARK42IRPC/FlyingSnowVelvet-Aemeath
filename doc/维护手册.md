...
- 福利 API 配置由 `lib/script/chat/welfare_api_config.py` 从 GitHub/Gitee 发布源并发测速获取密钥与地址，模型只信任认证后的 `/models` 结果。默认模型为 `agnes-2.0-flash`，“智力提升”开启后使用 `agnes-2.5-flash`；接口超时固定 10 秒，首次失败后最多重试 3 次。
