...
`config/config_*.py` 只存程序默认值，运行时禁止改写 Python 源文件。普通设置通过 `config.user_settings` 保存为版本化稀疏 JSON；密钥和登录态使用独立 secrets/state 路径。麦克风识别提供轻量 PCM16 自适应降噪：`microphone_denoise_enabled` 控制开关，`microphone_denoise_strength` 控制衰减强度，`microphone_noise_gate_threshold` 控制噪声门阈值；降噪在识别 Worker 内按 10ms 帧估计噪声底和响度变化率，稳定高响度按噪声衰减，突发变化保留短暂语音保持，自动语聊 RMS 判定使用处理后的音频。
