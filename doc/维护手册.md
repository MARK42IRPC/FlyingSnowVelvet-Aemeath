...
粒子覆盖层在一轮粒子开始前固定到虚拟桌面，存活期间禁止按联合包围盒移动或缩放顶层窗口。渲染使用 128px 二维固定矩形分块：每帧重建轻量空间索引，刷新上一帧和当前帧占用块的并集，`paintEvent()` 只查询脏区域命中的块并对跨块粒子去重。tick 插值起点使用更新前的逻辑坐标；大批量异步更新使用隔离快照，禁止后台线程修改正在绘制的对象。无存活粒子时跳过屏幕查询和计算提交。
