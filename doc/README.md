...
本文档目录以当前源码为准。版本号的唯一来源是 `config/version_info.py`。已完成的历史计划和清单不在主仓库继续维护。
...
| 设计或验收自包含 N 卡运行时 v2 | [CUDA 语音运行时 v2 自包含方案](CUDA语音运行时v2自包含方案.md) |
...
已完成的阶段计划直接由 Git 历史保留，不在 `doc/` 中保留可能误导为当前待办的副本。
